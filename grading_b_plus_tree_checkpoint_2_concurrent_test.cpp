...
void InsertTest1Call() {
  for (size_t iter = 0; iter < NUM_ITERS; iter++) {
    ...
    BufferPoolManagerInstance *bpm = new BufferPoolManagerInstance(50, disk_manager);
    // create b+ tree
...
void MixTest3Call() {
  for (size_t iter = 0; iter < NUM_ITERS; iter++) {
    ...
    int64_t size = 0;
    std::ofstream out("out.log", std::ios::trunc | std::ios::out);
    for (auto &pair : tree) {
      out << (pair.first).ToString() << std::endl;
      ASSERT_EQ((pair.first).ToString(), for_insert[size]);
      size++;
    }
